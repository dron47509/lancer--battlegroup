...
# Скрипт для узла Escort_wing_vbox
# Управление корпусами (Escort_wing) и их чертами (Feat) с сохранением в JSON.
...
#  ▸ Add_escort_wing_button      — создает/обновляет запись системы в JSON и кнопку в списке.
...
@onready var type_option_button   : OptionButton    = $Type_option_button
@onready var tags_edit            : TextEdit        = $Tag_text_edit
@onready var point_edit           : TextEdit        = $Point_text_edit
@onready var HP_edit              : TextEdit        = $HP_text_edit
@onready var tenacity_edit        : TextEdit        = $Tenacity_text_edit
@onready var range_edit           : TextEdit        = $Range_text_edit
...
@onready var add_escort_wing_button      : Button        = $Add_escort_wing_button

@onready var escorts_wings_list_container : VBoxContainer = $Escort_wing_vbox          # Контейнер кнопок систем
...
func _ready() -> void:
	_ensure_json()
	_populate_escort_wing_buttons()

	add_feat_button.pressed.connect(_add_feat_box)
	add_escort_wing_button.pressed.connect(_on_add_escort_wing_pressed)
...
# ============================================================================
#  ADD / UPDATE ESCORT_WING
# ============================================================================
func _on_add_escort_wing_pressed() -> void:
	var escort_wing_name := name_edit.text.strip_edges()
	if escort_wing_name == "":
		push_warning("Введите название системы.")
		return

	var escort_wing_dict := {
		"name": escort_wing_name,
		...
		"tenacity": tenacity_edit.text.strip_edges(),
	...
	var idx := _find_escort_wing_index(escort_wing_name)
	if idx == -1:
		data["escorts_wings"].append(escort_wing_dict)
		_create_escort_wing_button(escort_wing_dict)
	else:
		data["escorts_wings"][idx] = escort_wing_dict
	_save_json()
...
func _find_escort_wing_index(name:String) -> int:
	for i in range(data["escorts_wings"].size()):
		if data["escorts_wings"][i]["name"].to_lower() == name.to_lower():
			return i
	return -1
...
#  ESCORT_WING LIST UI
# ============================================================================
func _populate_escort_wing_buttons() -> void:
	_clear_children(escorts_wings_list_container)
	for h in data["escorts_wings"]:
		_create_escort_wing_button(h)

func _create_escort_wing_button(escort_wing:Dictionary) -> void:
	var btn := Button.new()
	btn.text = escort_wing["name"]
	btn.pressed.connect(func(): _load_escort_wing_to_form(escort_wing))
	escorts_wings_list_container.add_child(btn)

func _load_escort_wing_to_form(escort_wing:Dictionary) -> void:
	# Текстовые поля
	name_edit.text            = escort_wing["name"]
	type_option_button.selected = escort_wing["type"]
	tags_edit.text            = escort_wing["tags"]
	HP_edit.text              = escort_wing["hp"]
	point_edit.text           = str(escort_wing["points"])
	tenacity_edit.text        = escort_wing["tenacity"]
	range_edit.text           = escort_wing["range"]
	effect_edit.text          = escort_wing["effect"]
	discription_edit.text     = escort_wing["discription"]
	m_point_edit.text         = escort_wing["modification"]["point"]
	m_HP_edit.text            = escort_wing["modification"]["HP"]
	m_defence_edit.text       = escort_wing["modification"]["defence"]
	m_superheavy_edit.text    = escort_wing["modification"]["superheavy"]
	m_primary_edit.text       = escort_wing["modification"]["primary"]
	m_auxiliary_edit.text     = escort_wing["modification"]["auxiliary"]
	m_wing_edit.text          = escort_wing["modification"]["wing"]
	m_escort_edit.text        = escort_wing["modification"]["escort"]
	m_system_edit.text        = escort_wing["modification"]["system"]
	m_interdiction_edit.text  = escort_wing["modification"]["interdiction"]
	# ---------- FEATS ----------

	_clear_children_except(feats_parent, add_feat_button)

	var feats_arr : Array = escort_wing.get("feats", [])
